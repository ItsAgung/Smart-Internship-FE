...
const Monitoring = ({ title }) => {
  ...
  const [loadCert, setLoadCert] = useState({
    row: "",
    loading: false,
  });
  ...
  const handleFileChange = (event) => {
    ...
    if (!allowedTypes.includes(selectedFile?.type)) {
      toast.error("Only Doc/Docx File are allowed.");
      return;
    }
  ...
  async function getMonitoring() {
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_REACT_APP_API}/admin/monitoring`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      setDataMonitoring(response.data.data.user);
      setCertTemplate(response.data.data.sertifikat);
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
  ...
  async function submitSertifikat(e) {
    e.preventDefault();
    const template = {
      name: "sertifikat",
      file: file,
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_REACT_APP_API}/admin/sertifikat`,
        template,
        {
          headers: {
      ...
      document.getElementById("modal-sertifikat").close();
      Swal.fire({
        icon: "success",
  ...
  function handleSertifikat(row, id) {
    Swal.fire({
      title: "Beri Sertifikat",
      text: "Apakah anda yakin ingin memberikan sertifikat?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        setLoadCert({ row: row, loading: true });
        try {
          await axios.post(
            `${import.meta.env.VITE_REACT_APP_API}/admin/user/sertifikat`,
            {
              id: id,
            },
            {
          ...
          setLoadCert({ row: row, loading: false });
          getMonitoring();
        } catch (error) {
          setLoadCert({ row: row, loading: false });
          console.log(error);
    ...
    },
    {
      ...
      cell: (cell) => {
        return cell.getValue() === "null" ? (
          <span
            className={
              loadCert.row === cell.row.id && loadCert.loading
                ? "loading loading-dots loading-xs"
                : ""
            }
          >
            -
          ...
          <>
            <div className="dropdown dropdown-bottom dropdown-end">
              <label tabIndex={0} className="btn m-1">
                {cell.row.original.sertifikat === "null" ? (
                  <svg
                    width="29"
                    height="25"
                    viewBox="0 0 29 25"
              ...
              {cell.row.original.sertifikat === "null" && (
                ...
                >
                  <li>
                    <div
                      onClick={() =>
                        handleSertifikat(cell.row.id, cell.getValue())
                      }
                    >
                      Beri Sertifikat
                    </div>
    ...
    <>
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center mb-5 gap-3">
          {certTemplate && (
            <a
              href={
                import.meta.env.VITE_REACT_APP_API +
                "/certificate/" +
                certTemplate.location
              }
              target="_blank"
              rel="noreferrer"
              className="flex-none btn btn-ghost bg-[#4F6079] hover:bg-[#384353] btn-sm capitalize text-xs font-light"
            >
              <svg
          ...
          <button
            onClick={() => {
              document.getElementById("modal-sertifikat").showModal();
            }}
            className="btn btn-ghost bg-[#4F6079] hover:bg-[#384353] text-white w-40 btn-sm capitalize text-xs font-light"
          >
            + Template Sertifikat
          </button>
        </div>
      ...
      {/* Modal Tambah Template Sertifikat */}
      <dialog id="modal-sertifikat" className="modal">
        <div className="modal-box overflow-hidden">
          ...
          <form
            onSubmit={submitSertifikat}
            className="mt-5 flex flex-col justify-between"
          >
            <div>
              <div className="mb-2 block">
                <Label htmlFor="file" value="Upload Template Sertifikat:" />
              </div>
              <input
                id="file"
                type="file"
              ...
              <label className="label">
                <span className="label-text-alt">Format .doc / .docx</span>
              </label>
            </div>
